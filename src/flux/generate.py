...
def prepare_params(
    ...
    num_inference_steps: int = 28, # 原代码28
...
def generate(
    ...
    **params: dict,
    ...
    if use_condition:
        # assert len(conditions) <= 1, "Only one condition is supported for now."# 作者限制一个condition
        if not default_lora:
            pipeline.set_adapters(conditions[0].condition_type) # 默认所有条件的条件类型都一样
            
        # 原代码，无condition_index
        ...
        # 加入condition_index，区分不同condition之间的y轴偏移量
        for i, condition in enumerate(conditions):
            tokens, ids, type_id = condition.encode(self, condition_index=i)
