...
class Condition(object):
    ...
    def encode(self, pipe: FluxPipeline, condition_index: int = 0) -> Tuple[torch.Tensor, torch.Tensor, int]:
        """
        Encodes the condition into tokens, ids and type_id.
        
        Args:
            pipe: FluxPipeline instance
            condition_index: 条件的序号，从0开始。影响y轴方向的偏移量
        """
        if self.condition_type in [
            "depth",
            "canny",
        ...
        if self.position_delta is None and self.condition_type == "subject":
            # 根据条件序号计算y轴偏移量：-1/16, -1/8, -3/16, -1/4, ...
            y_offset = -(condition_index + 1) * self.condition.size[0] // 16
            self.position_delta = [0, y_offset]
